...
const main = async () => {
    ...
    // Start Bid
    await nftContract.methods.approve(auctionContract._address, nftIdToAuction).send({ from: owner })
    await auctionContract.methods.start(allowAuctionFor).send({ from: owner, gas: 2000000 })

    const nftId = await nftContract.methods.walletOfOwner(auctionContract._address).call()
    console.log(`${auctionContract._address} (Auction Contract) now owns NFT ${nftId}\n`)
    ...
    console.log(`Highest Bid: ${web3.utils.fromWei(highestBid.toString(), 'ether')} ether from ${highestBidder}\n`)
    ...
    const bidderCollection = await nftContract.methods.walletOfOwner(bidder2).call()
    ...
    console.log(`Original owner balance before auction | ${web3.utils.fromWei(balanceBefore.toString(), 'ether')} ether`)
    console.log(`Original owner balance after auction  | ${web3.utils.fromWei(balanceAfter.toString(), 'ether')} ether\n`)
}
